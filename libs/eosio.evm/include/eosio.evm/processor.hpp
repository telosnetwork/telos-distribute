...
namespace eosio_evm {
  ...
  public:
    ...
    ExecResult process_transaction(const Account& caller);
    ...
    // EIP-2929
    void access_list_print(); 
    void access_list_add_address(const Address& addr);
    void access_list_add_slot(const Address& addr, const uint256_t& slot);
    bool access_list_contains_address(const Address& addr);
    bool access_list_contains(const Address& addr, const uint256_t& slot);
    void access_list_remove_address(const Address& addr);
    void access_list_remove_slot(const Address& addr, const uint256_t& slot);
    ...
    bool process_sstore_gas(uint256_t slot, uint256_t original_value, uint256_t current_value, uint256_t new_value);
    ...
    BN adjusted_exponent_length(const std::vector<uint8_t>& input, const BN& el, const BN& bl);
    BN mult_complexity(const BN& len);
    BN read_input(const std::vector<uint8_t>& input, uint64_t start, const uint64_t size);
